...
class CndRegStagesEnsembleMix:
    ...
    def train(
        ...
        for model_num, (cnd_var_name, expl_var_name) in enumerate(initial_model_names):
            ...
            intermediate_ground_truths[model_num][0] = self.__ratio_difference(current_ground_truth, prediction)
        ...
        for model_num in range(self.__num_candidates):
            for stage in range(1, self.__num_stages):
                ...
                intermediate_predictions[model_num][stage] = intermediate_predictions[model_num][stage - 1] * self.__revert_ratio_difference(prediction)
                intermediate_ground_truths[model_num][stage] = self.__ratio_difference(ground_truth, intermediate_predictions[model_num][stage])
    ...
    def predict_all(
        ...
        df_pred = df.copy()
        predictions = np.zeros((self.__num_candidates, len(df_pred)))

        for model_num in range(self.__num_candidates):

            cnd_var_name = self.__model[model_num][0].cnd_var_name
            expl_var_name = self.__model[model_num][0].expl_var_name

            single_model_prediction = self.__model[model_num][0].predict(
            ...
            for stage in range(1, self.__num_stages):
                ...
                ratio_pred = self.__model[model_num][stage].predict(cnd_var_data = df_pred[cnd_var_name].to_numpy(), expl_var_data = df_pred[expl_var_name].to_numpy())
                ratio_pred = self.__revert_ratio_difference(ratio_pred)
                single_model_prediction = single_model_prediction * ratio_pred

            predictions[model_num] = single_model_prediction
    ...
    def __ratio_difference(
        self,
        ground_truth: np.ndarray,
        prediction: np.ndarray
    ) -> np.ndarray:
        
        idxs = np.where(ground_truth == 0.0)
        ground_truth[idxs] = 0.001 * (np.max(ground_truth) - np.min(ground_truth))
        
        difference = 1.0 - (prediction - ground_truth) / ground_truth

        difference[difference > 1.1] = 1.1
        difference[difference < 1 / 1.1] = 1 / 1.1

        idxs_below_one = np.where(difference < 1.0)
        idxs_above_one = np.where(difference >= 1.0)
        below_zeros_diff = (1.0 / difference[idxs_below_one]) - 1.0

        difference[idxs_above_one] = difference[idxs_above_one] - 1.0
        difference[idxs_below_one] = -below_zeros_diff

        return difference

    def __revert_ratio_difference(
        self,
        ratio_diff: np.ndarray
    ) -> np.ndarray:

        idxs_below_zero = np.where(ratio_diff < 0.0)
        idxs_above_zero = np.where(ratio_diff >= 0.0)

        below_zero_converted = 1.0 / (1.0 + np.abs(ratio_diff[idxs_below_zero]))
        above_zero_converted = 1.0 + ratio_diff[idxs_above_zero]

        reverted = np.zeros(ratio_diff.shape[0])
        reverted[idxs_below_zero] = below_zero_converted
        reverted[idxs_above_zero] = above_zero_converted
